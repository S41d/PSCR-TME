...
void fillScene(Scene &scene, default_random_engine &re) {
  ...
  // on remplit la scene de spheres colorees de taille position et couleur
  // aleatoire
  ...
  for (int i = 0; i < NBSPHERES; i++) {
    ...
    scene.add(Sphere({50 + distribd(re), 50 + distribd(re), 120 + distribd(re)}, double(distrib(re) % 30) + 3.0,
                     Color::random()));
  }
...
class DrawJob : public Job {
 ...
 public:
  DrawJob(Scene::screen_t screen, int x, int y, Scene scene, Color *pixels, vector<Vec3D> lights)
      : screen(screen), x(x), y(y), scene(scene), pixels(pixels), lights(lights) {}

  void run() override {
    // le point de l'ecran par lequel passe ce rayon
    // const value_type & || const pr::Vec3D
...
int main() {
  ...
  for (int x = 0; x < scene.getWidth(); x++) {
    for (int y = 0; y < scene.getHeight(); y++) {
      // std::cout << "submiting job : x=" << x << ", y=" << y << std::endl;
      p.submit(new DrawJob(screen, x, y, scene, pixels, lights));
    }
  }
  p.stop();
