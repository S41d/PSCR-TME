...
public:
  ...
  Color operator*(double ratio) {
    ...
    return Color(char(double(r) * ratio), char(double(g) * ratio),
                 char(double(b) * ratio));
  }
  // sommer deux couleurs (RGB donc additif)
  Color operator+(const Color &c2) const {
    return Color(std::min(255, (int)r + c2.r), std::min(255, (int)g + c2.g),
                 std::min(255, (int)b + c2.b));
  }
  ...
  static Color random() {
    ...
    std::uniform_int_distribution<int> distrib(
        30, 255); // eviter les teintes trop sombres
